...
CREATE FUNCTION update_desk (json, integer) RETURNS desk_view AS $$
  UPDATE "desk" SET
    "background" = CASE
...
CREATE FUNCTION update_folder (json, integer) RETURNS folder_view AS $$
  UPDATE "folder" SET 
    "name" = COALESCE($1->>'name', "name"),
    "icon" = COALESCE($1->>'icon', "icon"),
    "favorite" = COALESCE(($1->>'favorite')::boolean, "favorite"),
    "folder_id" = CASE
      WHEN ($1->>'folder_id')::integer = 0
        THEN NULL
        ELSE COALESCE(($1->>'folder_id')::integer, "folder_id")
    END
  WHERE "id" = $2
  RETURNING "id", "name", "icon", "favorite", "account_id" AS "accountId", "folder_id" AS "folderId";
$$ LANGUAGE SQL STRICT;
...
CREATE FUNCTION update_file (json, integer) RETURNS file_view AS $$
  UPDATE "file" SET
    "name" = COALESCE($1->>'name', "name"),
