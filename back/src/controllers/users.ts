...
export default {
  ...
  updateOne: async (req: Request, res: Response) => {
    ...
    if (req.body.avatar === 'deleted') {
      ...
      if (user.avatar) {
        const to = path.join(process.cwd(), 'static/users/avatars', user.avatar);
      ...
      writeFileSync(path.join(process.cwd(), `static/users/avatars/${req.body.avatar}`), req.file.buffer);
    }
